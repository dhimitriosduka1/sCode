...
- **Interactive Job Dependency Submission**: Added a dedicated "Submit with Dependency" workflow (`$(workflow)`) next to the main Play buttons in the sidebar and editor titles. The main Play buttons submit jobs immediately (preserving the fast, 1-click UX), while the Dependency buttons guide users through step-by-step active job selection, custom ID inputs, and dependency types (`afterok`, `afterany`, `after`, `afternotok`) directly and efficiently.
